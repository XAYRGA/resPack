...
namespace xayrga.byteglider
{
    public unsafe class bgReader : BinaryReader
    {
        ...
        public void Align(int alignment = 16, BGAlignDirection direction = BGAlignDirection.FORWARD, bool forceAdvance = false)
        {
            var currentPos = BaseStream.Position;
            var remainder = currentPos % alignment;

            if (remainder == 0 && !forceAdvance) // We're already aligned.
                return;        

            if (direction == BGAlignDirection.FORWARD)
